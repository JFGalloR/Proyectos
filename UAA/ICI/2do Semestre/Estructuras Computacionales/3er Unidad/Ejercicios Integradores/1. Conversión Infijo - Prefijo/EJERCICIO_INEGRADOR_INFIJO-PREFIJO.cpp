/*===========================================================================================================
	EJERCICIO INTEGRADOR UNIDAD III. Conversión se Notación de Infijo - Prefijo.
  ---------------------------------------------------------------------------------------------------------
	• OBJETIVO:
		Realizar un algoritmo para la conversión de expresión de Notación de Infijo-Prefijo implementado 
		en PILAS.
  ---------------------------------------------------------------------------------------------------------
    • ELABORADORES:
...
//======================================================== FUNCIÓN PARA CONVERTIR LA EXPRESIÓN A PREFIJO =====
/*-----------------------------------------------------------------------------------------------------------
	Función que nos hace el proceso de convertir una expresión en infija a prefija.
  ---------------------------------------------------------------------------------------------------------*/
string conversion(Nodo *&pila, string &exp)
{
	...
	// La expresión está en orden inverso, por lo que solo basta con invertirla.
	exp = cad_aux;
	for(int i=0; i < cad_aux.size(); i++)
	{
		exp[i] = cad_aux[cad_aux.size()-1-i];
	}
...
main()
{
	...
	do
	{
		// Texto de control.
		system("cls");
		color(ROJ, GRIC);cout<<"                                    CONVERSIÓN (Infijo a Prefijo)                                    "<<endl<<endl<<endl;
		...
		if(cadMayus(exp) != "SALIR")
		{
			...
			if(comprobar(exp))
			{
				...
				// Se imprime la expresión en prefijo.
				color(GRIC, ROJ);cout<<endl<<endl<<"\t==== PREFIJO ======================================"<<endl<<endl;
